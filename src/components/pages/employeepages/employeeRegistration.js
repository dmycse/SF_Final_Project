				...
				{({isSubmitting, handleReset, status, dirty, errors, touched}) => (
					<Form className='form'>
						...
						<div className='form__buttons'>
							<button type='button' disabled={isSubmitting || !dirty} onClick={handleReset} className='form__btn'>Очистить</button>
							<button type='submit' disabled={isSubmitting || !dirty} className='form__btn'>Подтвердить</button>
						</div>
					</Form>
					)
				}
